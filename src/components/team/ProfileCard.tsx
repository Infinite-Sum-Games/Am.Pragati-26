...
import { IoDisc, IoMusicalNotes, IoSparkles } from "react-icons/io5";
...
const ProfileCard = ({
	...
	const details = year
		? `B.Tech ${dept || ""} ${numberToRoman(year)} Year`
		: (dept ?? "");
		...
		>
			...
			<div className="absolute inset-0 bg-linear-to-br from-gray-700 via-gray-800 to-gray-900">
				...
				<div className="absolute top-0 left-0 right-0 h-8 bg-linear-to-b from-gray-600 to-gray-700 border-b-2 border-black flex items-center justify-between px-3">
					...
					<div className="text-[#2de2e6] text-xs opacity-75">{badgeIcon}</div>
				</div>
				...
				<div className="absolute top-12 left-4 right-4 h-[58%] bg-black border-4 border-gray-600 rounded-md overflow-hidden shadow-[inset_0_4px_8px_rgba(0,0,0,0.8)]">
					...
					<div
						...
						style={{
							backgroundImage:
								"repeating-linear-gradient(0deg, rgba(0,0,0,0.5) 0px, transparent 1px, transparent 2px)",
						}}
					/>
					...
					<div className="relative pt-4">
						...
						<div className="relative flex flex-col items-center justify-center gap-1 min-h-12">
							<div className="flex flex-col items-center justify-center gap-1 transition-opacity duration-300 group-hover:opacity-0">
								<p className="text-[#2de2e6] text-xs font-bold uppercase tracking-wider drop-shadow-[0_0_5px_rgba(45,226,230,0.5)] text-center">
									{role}
								</p>
								<p className="text-gray-400 text-[9px] tracking-widest font-mono text-center">
									{details}
								</p>
							</div>
				...
				<div className="relative flex gap-3 bg-linear-to-b from-gray-700 to-gray-800 px-4 py-2 rounded-sm backdrop-blur-sm border-2 border-gray-900 pointer-events-auto transform translate-y-4 group-hover:translate-y-0 transition-transform duration-300 shadow-[0_4px_0_rgba(0,0,0,0.8)]">
					...
					{linkedin && (
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="text-gray-300 hover:text-[#00d9ff] hover:scale-110 transition-all p-1 drop-shadow-[0_0_8px_currentColor]"
						>
							<FaLinkedin size={18} />
						</a>
					)}
					{instagram && instagram !== "-" && (
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="text-gray-300 hover:text-[#ff4fd8] hover:scale-110 transition-all p-1 drop-shadow-[0_0_8px_currentColor]"
						>
							<FaInstagram size={18} />
						</a>
					)}
					{contactEmail && (
						<a
							...
							className="text-gray-300 hover:text-[#ffa726] hover:scale-110 transition-all p-1 drop-shadow-[0_0_8px_currentColor]"
						>
							<FaEnvelope size={18} />
						</a>
					)}
					{github && (
						<a
							...
							target="_blank"
							rel="noreferrer"
							className="text-gray-300 hover:text-[#b25cff] hover:scale-110 transition-all p-1 drop-shadow-[0_0_8px_currentColor]"
						>
							<FaGithub size={18} />
						</a>
					)}
